Configuring MailHogPlus
===================

You can configure MailHogPlus using command line options or environment variables:
...
| MH_HOSTNAME         | -hostname       | MailHogPlus.example | Hostname to use for EHLO/HELO and message IDs
...
| MH_MONGO_DB         | -mongo-db       | MailHogPlus         | MongoDB database name for message storage
...
If you're using MailHogPlus behind a reverse proxy, e.g. nginx, make sure WebSocket connections
are also supported and configured - see [this issue](https://github.com/mailhog/MailHogPlus/issues/117) for information.
