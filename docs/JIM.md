...
Jim is the MailHogPlus Chaos Monkey, inspired by Netflix.

You can invite Jim to the party using the `invite-jim` flag:

    MailHogPlus -invite-jim
...
of it happening to 0, e.g. to disable connection rate limiting:
    ...
    MailHogPlus -invite-jim -jim-linkspeed-affect=0

### Examples

Always rate limit to 1 byte per second:

    MailHogPlus -invite-jim -jim-linkspeed-affect=1 -jim-linkspeed-max=1 -jim-linkspeed-min=1

Disconnect clients after approximately 5 commands:

    MailHogPlus -invite-jim -jim-disconnect=0.2

Simulate a mobile connection (at 10-100kbps) for 10% of clients:

    MailHogPlus -invite-jim -jim-linkspeed-affect=0.1 -jim-linkspeed-min=1250 -jim-linkspeed-max=12500
